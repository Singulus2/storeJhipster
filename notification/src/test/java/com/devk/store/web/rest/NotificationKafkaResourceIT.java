...
import com.devk.store.config.EmbeddedKafka;
import com.devk.store.config.KafkaSseConsumer;
import com.devk.store.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class NotificationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/notification-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/notification-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/notification-kafka/unregister"));
                return;
            }
        }
